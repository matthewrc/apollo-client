    ...
    QueryManager.prototype.getObservableQueries = function (include) {
        ...
        if (__DEV__ && queryNamesAndDocs.size) {
            queryNamesAndDocs.forEach(function (included, nameOrDoc) {
                if (!included) {
                    invariant.warn("Unknown query " + (typeof nameOrDoc === "string" ? "named " : "") + JSON.stringify(nameOrDoc, null, 2) + " requested in refetchQueries options.include array");
                }
            });
        }
        return queries;
    ...
    QueryManager.prototype.fetchQueryByPolicy = function (queryInfo, _a, networkStatus) {
        ...
        var resultsFromCache = function (diff, networkStatus) {
            ...
            if (__DEV__ &&
                ...
                !equal(data, {}) &&
                !returnPartialData) {
                invariant.debug("Missing cache result fields: " + diff.missing.map(function (m) { return m.path.join('.'); }).join(', '), diff.missing);
            }
